	...
	rs.open sorgu, sbsv5, 1, 3
		if rs.recordcount > 0 then
			for zi = 1 to rs.recordcount
					cariAd	=	rs("cariAd")
					cariID	=	rs("cariID")
				Response.Write "<div class=""btn btn-secondary rounded mt-3 text-left"">"
					Response.Write "<div onclick=""working('DIV2','30px','30px');$('#DIV2').load('/fason/fason_verilenIs.asp',{cariID:" & cariID & ", islem:'" & islem & "'})"">" & cariAd & "</div>"
				Response.Write "</div>"
			rs.movenext
			next
		end if
